    ...
    public class GltfWriter : IGltfWritable
    {
        ...
        struct AttributeData
        {
                ...
                )
            {
                ...
                Size = GetAttributeSize(descriptor.format) * descriptor.dimension;
            }
        ...
        /// <inheritdoc />
        public static bool TransformIsIdentity = false;
        public uint AddNode(
        ...
        )
        {
            ...
            //UTG
            //bool TransformIsIdentity = false;// true;
        ...
        static void AssignMaterialsToMesh(int[] materialIds, Mesh mesh)
        {
            ...
            //UTG->Limit visible objects to applied materials to mimic Unity behavior
            if (materialIds.Length < mesh.primitives.Length)
            {
                var OldPrimitives = mesh.primitives;
                mesh.primitives = new MeshPrimitive[materialIds.Length];
                for (var i = 0; i < materialIds.Length; i++)
                {
                    mesh.primitives[i] = OldPrimitives[i];
                }
            }
        }
        ...
        public static int GetAssumedOffset(VertexAttribute target, int stream, VertexAttributeDescriptor[] layout)
        {
            int offset = 0;

            for(int i=0; i<layout.Length; i++)
            {
                VertexAttributeDescriptor desc = layout[i];
                if ( desc.stream == stream )
                {
                    if (desc.attribute == target)
                        return offset;

                    offset += GetAttributeSize(desc);
                }
            }

            Debug.LogWarning($"Attribute {target} not found in layout.");
            return -1;
        }

        public static int GetAttributeSize(VertexAttributeDescriptor desc)
        {
            int bytesPerComponent = desc.format switch
            {
                VertexAttributeFormat.Float32 => 4,
                VertexAttributeFormat.Float16 => 2,
                VertexAttributeFormat.UNorm8 or
                VertexAttributeFormat.SNorm8 or
                VertexAttributeFormat.UInt8 or
                VertexAttributeFormat.SInt8 => 1,
                VertexAttributeFormat.UNorm16 or
                VertexAttributeFormat.SNorm16 or
                VertexAttributeFormat.UInt16 or
                VertexAttributeFormat.SInt16 => 2,
                _ => throw new System.NotSupportedException($"Unsupported format: {desc.format}")
            };

            return bytesPerComponent * desc.dimension;
        }
        private static readonly VertexAttribute[] ProceduralMeshOrder =
        {
        ...
        private static int GetOrder(VertexAttribute attr)
        {
            int idx = Array.IndexOf(ProceduralMeshOrder, attr);
            return idx >= 0 ? idx : int.MaxValue; // unknown attributes go last
        }

        public static void SortForProceduralMesh(VertexAttributeDescriptor[] attributes)
        {
            Array.Sort(attributes, (a, b) =>
            {
                int orderA = GetOrder(a.attribute);
                int orderB = GetOrder(b.attribute);
                return orderA.CompareTo(orderB);
            });
        }
        public static bool DetectVertexAttributeMismatches(UnityEngine.Mesh mesh, VertexAttributeDescriptor[] vertexAttributes)
        {
            var meshDataArray = UnityEngine.Mesh.AcquireReadOnlyMeshData(mesh);
            ...
            unsafe
            {
                ...
                {
                    var desc = vertexAttributes[i];
                    NativeArray<byte> vertexData = meshData.GetVertexData<byte>(desc.stream);
                    byte* basePtr = (byte*)vertexData.GetUnsafeReadOnlyPtr();
                    int offset = GetAssumedOffset( desc.attribute, desc.stream, vertexAttributes );

                    switch (desc.attribute)
                    {
                        case VertexAttribute.Position:
                            {
                                float* ptr = (float*)(basePtr + offset);
                                Vector3 v = new Vector3(ptr[0], ptr[1], ptr[2]);
                                if (v != vertices[0])
                                { 
                                    Debug.LogError($"[{mesh.name}] Position mismatch: mesh.vertices[0]={vertices[0]} vs buffer={v}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        case VertexAttribute.Normal:
                            {
                                float* ptr = (float*)(basePtr + offset);
                                Vector3 n = new Vector3(ptr[0], ptr[1], ptr[2]);
                                if (n != normals[0])
                                {
                                    Debug.LogError($"[{mesh.name}] Normal mismatch: mesh.normals[0]={normals[0]} vs buffer={n}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        case VertexAttribute.Tangent:
                            {
                                float* ptr = (float*)(basePtr + offset);
                                Vector4 t = new Vector4(ptr[0], ptr[1], ptr[2], ptr[3]);
                                if (t != tangents[0])
                                {
                                    Debug.LogError($"[{mesh.name}] Tangent mismatch: mesh.tangents[0]={tangents[0]} vs buffer={t}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        case VertexAttribute.Color:
                            {
                                ...
                                Color32 c = new Color32(ptr[0], ptr[1], ptr[2], ptr[3]);
                                if (!c.Equals(colors32[0]))
                                {
                                    Debug.LogError($"[{mesh.name}] Color mismatch: mesh.colors32[0]={colors32[0]} vs buffer={c}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        case VertexAttribute.TexCoord7:
                            {
                                float* ptr = (float*)(basePtr + offset);
                                Vector2 uvValue = new Vector2(ptr[0], ptr[1]);

                                Vector2 uvCompare = desc.attribute switch
                                {
                                    VertexAttribute.TexCoord0 => uv0[0],
                                    VertexAttribute.TexCoord1 => uv1[0],
                                    VertexAttribute.TexCoord2 => uv2[0],
                                    VertexAttribute.TexCoord3 => uv3[0],
                                    VertexAttribute.TexCoord4 => uv4[0],
                                    VertexAttribute.TexCoord5 => uv5[0],
                                    VertexAttribute.TexCoord6 => uv6[0],
                                    VertexAttribute.TexCoord7 => uv7[0],
                                    _ => Vector2.zero
                                };

                                if (uvValue != uvCompare)
                                {
                                    Debug.LogError($"[{mesh.name}] {desc.attribute} mismatch: mesh uv[0]={uvCompare} vs buffer={uvValue}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        case VertexAttribute.BlendIndices:
                            {
                                byte* ptr = basePtr + offset;
                                Color32 indices = new Color32(ptr[0], ptr[1], ptr[2], ptr[3]);
                                BoneWeight bw = boneWeights[0];
                                Color32 bwIndices = new Color32(
                                    (byte)bw.boneIndex0,
                                    (byte)bw.boneIndex1,
                                    (byte)bw.boneIndex2,
                                    (byte)bw.boneIndex3
                                ...
                                if (!indices.Equals(bwIndices))
                                {
                                    Debug.LogError($"[{mesh.name}] BlendIndices mismatch: boneWeights[0]={bwIndices} vs buffer={indices}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        case VertexAttribute.BlendWeight:
                            {
                                float* ptr = (float*)(basePtr + offset);
                                Vector4 weights = new Vector4(ptr[0], ptr[1], ptr[2], ptr[3]);
                                BoneWeight bw = boneWeights[0];
                                Vector4 bwWeights = new Vector4(bw.weight0, bw.weight1, bw.weight2, bw.weight3);

                                if (weights != bwWeights)
                                {
                                    Debug.LogError($"[{mesh.name}] BlendWeight mismatch: boneWeights[0]={bwWeights} vs buffer={weights}");
                                    return true;
                                }
                            }
                            break;
                        ...
                        default:
                            // ignore unhandled attributes silently
                            break;
                    }
                }
            }
        ...
        async Task BakeMesh(int meshId, IMeshData meshData)
        {
            ...
            if ( DetectVertexAttributeMismatches( uMesh, vertexAttributes ) )
            {
                SortForProceduralMesh( vertexAttributes );
            }
            ...
            foreach (var attribute in vertexAttributes)
            {
                ...
                int OutDimension = attribute.dimension;
                VertexAttributeFormat OutFormat = attribute.format;
                ...
                {
                    //OutFormat = VertexAttributeFormat.Float32;
                }

                var attributeElementSize = GetAttributeSize(attribute.format);
                var InputAttributeSize = attribute.dimension * attributeElementSize;
                var OutElementSize = GetAttributeSize(OutFormat);
                var OutputAttributeSize = OutDimension * OutElementSize;
                ...
                attrData.Size = GetAttributeSize(OutFormat) * OutDimension;

                inputStrides[attribute.stream] += InputAttributeSize;
                ...
                outputStrides[attribute.stream] += OutputAttributeSize;
                ...
                var accessor = new Accessor
                {
                    ...
                    componentType = Accessor.GetComponentType(OutFormat),
                    count = vertexCount,
                };
                
                accessor.SetAttributeType(Accessor.GetAccessorAttributeType(OutDimension));
                ...
                switch (vertexAttribute)
                {
                    ...
                    case VertexAttribute.BlendWeight:
                        await ConvertSkinWeightsAttribute(
                            ...
                            inputStreams[attrData.descriptor.stream],
                            outputStreams[attrData.descriptor.stream],
                            attrData.descriptor.format//VertexAttributeFormat.Float32//
                        );
                        break;
                    case VertexAttribute.BlendIndices:
            ...
            //float2[] UVs = new float2[100];
            //unsafe
            //{
            //    int TestVerts = 100;                
            //    for (int i = 0; i < vertexCount && i < TestVerts; i++)
            //    {
            //        byte* InPtr = (byte*)inputStreams[0].GetUnsafeReadOnlyPtr() + i * inputStrides[0];
            //        byte* OutPtr = (byte*)outputStreams[0].GetUnsafeReadOnlyPtr() + i * outputStrides[0];
            //
            //        float* InPtrFloat = (float*)InPtr;
            //        float* OutPtrFloat = (float*)OutPtr;
            //        UVs[i] = new float2( OutPtrFloat[11], OutPtrFloat[12] );
            //    }
            //}
        ...
        static async Task ConvertSkinWeightsAttribute(
            ...
            VertexAttributeFormat OutFormat
        )
        {
            var job = ConvertSkinWeightsAttributeJob(attrData, inputByteStride, outputByteStride, vertexCount, inputStream, outputStream, OutFormat );
            while (!job.IsCompleted)
            {
                await Task.Yield();
            }
        ...
        )
        {
            var job = new ExportJobs.ConvertSkinWeightsJob
            {
                ...
                outputByteStride = outputByteStride,
                output = (byte*)outputStream.GetUnsafePtr() + attrData.outputOffset,
                dimension = attrData.descriptor.dimension,
                InFormat = attrData.descriptor.format,
                OutFormat = OutFormat
            }.Schedule(vertexCount, k_DefaultInnerLoopBatchCount);
            return job;
        }
        ...
        )
        {
            var job = new ExportJobs.ConvertSkinIndicesJob
            {
                ...
                dimension = indicesAttrData.descriptor.dimension
            }.Schedule(vertexCount, k_DefaultInnerLoopBatchCount);
            return job;
        }
