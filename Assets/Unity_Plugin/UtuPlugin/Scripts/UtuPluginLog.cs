...
public class UtuLog
{
    ...
    public static string GetUnityExportLogFilePath()
    {
		return UtuPluginPaths.GetCurrentExportFolderPath() + "UnityExport.log";
    }
    ...
    public static void PrintIntoLogFile(string message)
	{
		try
		{
            File.AppendAllText(GetUnityExportLogFilePath(), message + "\n");
        }
		catch 
		{ 
			Debug.LogError("UTU Error - Failed to write export log file. Does Unity have the write permission to this file: '" + GetUnityExportLogFilePath() + "'");
        }
    }
	...
	public static void InitializeNewLog()
	{
		UtuLog.Log("Log File Path: \"" + GetUnityExportLogFilePath() + "\"");
		UtuLog.Log("Utu Plugin Version: " + UtuPlugin.GetUtuPluginVersion());
	}
